...
export default function GroupPage() {
  ...
  const handleCopy = () => {
    if (!group) return;
    // 그룹 데이터를 URL에 인코딩해서 공유 — 받는 쪽 localStorage에 없어도 참가 가능
    ...
    const inviteUrl = `${window.location.origin}/friends?invite=${btoa(payload)}`;
    ...
    if (navigator.clipboard && window.isSecureContext) {
      navigator.clipboard.writeText(inviteUrl).then(markCopied).catch(fallback);
    } else {
      fallback();
    }
  };
